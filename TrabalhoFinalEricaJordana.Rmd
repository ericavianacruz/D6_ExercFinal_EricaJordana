...
#### 3.2 Qual percentual das reclamações do segmento Transporte Áereo em relacao as reclamações da área Transporte? 

#### 3.3 Quais são as empresas com mais reclamações?

#### 3.4 Como está o uso do consumidor.gov pa o segmento de transporte áereo nos anos analisados?

#### 3.5 Como se dá o encerramento das reclamações, são resolvidas? 
...
```{r, warning=FALSE}
setwd("E:/ENAP_Espe/D6_ColEAnDados/D6_ExercFinal_EricaJordana/dados_brutos/")
...
reclamacoes_area <- reclamacoes_area %>% arrange(desc(prop))
...
## 6. Quantidade de reclamações pelas principais empresas
```{r, echo=TRUE}
empresas_ano_principais %>% ggplot (aes (x= "", y = Quantidade_Reclamacoes, fill = Nome_Fantasia)) +
    theme_minimal() +
    geom_bar (stat = "identity") +
    labs(title = "Quantidade de reclamações pelas principais empresas", x = "Quantidade", fill = "Empresa") +
